...
import 'package:znews/bloc/get_top_headlines_bloc.dart';
...
class HeadlingSliderWidget extends StatefulWidget {
  ...
  return SizedBox( height: 200.0, child: buildErrorWidget(snapshot.data!.error));
  }
  return _buildHeadlineSliderWidget(snapshot.data!);
  } else if (snapshot.hasError) {
  return SizedBox( height: 200.0, child: buildErrorWidget(snapshot.data!.error));
  } else {
  return Shimmer.fromColors(child: _buildHeadlineSliderShimmerWidget(),
  baseColor: Colors.grey[300]!, highlightColor: Colors.grey[100]!);
  }
  },
  );
  }
  Widget _buildHeadlineSliderShimmerWidget() {
  return Container(
  height: 200.0,
  ...
  getExpenseSliders(List<ArticleModle> articles) {
  return articles
      .map((article) =>
  GestureDetector(
  onTap: () {
  ...
  AssetImage("assets/img/placeholder.jpg") : NetworkImage(
  article.img) as ImageProvider),
  )),
  Container(
  decoration: BoxDecoration(
  borderRadius: BorderRadius.all(
  ...
  right: 10.0,
  child: Text(
  timeUntil(DateTime.parse(article.data)),
  style: const TextStyle(
  color: Colors.white54,
  fontSize: 9.0),
  )),
